...
import { Calendar, Copy, FileText, KeyRound, Link as LinkIcon, Loader2, Mail, PlusCircle, Trash2 } from 'lucide-react';
...
import {
  ...
  makeDefaultFormConfig,
  buildCronConfigFromProviderConfig,
  buildFormConfigFromProviderConfig,
  serializeCronConfig,
  serializeFormConfig,
...
export function WorkflowTriggerModal({
  ...
  const [formConfig, setFormConfig] = useState<FormConfigState>(() => makeDefaultFormConfig());
  ...
  const formTrigger = useMemo(
    () => triggers.find((trigger) => trigger.key === FORM_TRIGGER_KEY),
    [triggers],
  );
  const triggerOptions = useMemo(() => {
    ...
    if (formTrigger) {
      options.push({
        ...
        title: formTrigger.title ?? 'Form Submission',
        description:
          formTrigger.description ?? 'Accept public form submissions that trigger this workflow.',
      });
    }
    return options;
  }, [genericTrigger, gmailTrigger, cronTrigger, formTrigger]);
  ...
  const canCreateForm = Boolean(workflowId && formTrigger && workflowInputEntries.length);
  ...
  const handleCreateFormSubscription = async () => {
    if (!workflowId || !formTrigger) {
      toast.error('Save the workflow before adding triggers');
      return;
    }
    if (!workflowInputEntries.length) {
      toast.error('Form trigger requires workflow inputs', {
        description: 'Add Input blocks to define the form fields.',
      });
      return;
    }
    setCreatingTriggerKey(FORM_TRIGGER_KEY);
    try {
      const providerConfig = serializeFormConfig(formConfig);
      // Form triggers use auto-binding - bindings are empty and generated by backend
      await createSubscription({
        workflow_id: workflowId,
        trigger_key: formTrigger.key,
        provider_config: providerConfig,
        bindings: {},
        enabled: true,
      });
  ...
  const renderSubscriptionCard = (subscription: TriggerSubscriptionResponse) => {
    ...
    const isFormTrigger = subscription.trigger_key === FORM_TRIGGER_KEY;
    ...
    const renderFormSummary = () => {
      ...
      const description = providerConfig['description']
        ? String(providerConfig['description'])
        : null;
      const publicFormUrl = `${window.location.origin}/forms/${subscription.subscription_id}`;
      const apiSubmitUrl = `${getBackendBaseUrl()}/v1/forms/${subscription.subscription_id}/submit`;
      return (
        <div className="flex flex-col gap-2 rounded-md border border-dashed border-muted p-3 bg-muted/30 text-sm">
          <div className="flex items-center gap-2 font-medium">
          ...
          <div className="space-y-2">
            <div>
              <p className="text-xs font-medium mb-1">Public form URL:</p>
              <code className="text-[10px] break-all bg-background px-2 py-1 rounded block">{publicFormUrl}</code>
            </div>
            <div>
              <p className="text-xs font-medium mb-1">API submission URL (POST):</p>
              <code className="text-[10px] break-all bg-background px-2 py-1 rounded block">{apiSubmitUrl}</code>
            </div>
          </div>
          <div className="flex gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => handleCopy(publicFormUrl, 'Public form URL')}
            >
    ...
    return (
      ...
      >
        ...
        {isFormTrigger && renderFormSummary()}
  ...
  const renderFormForm = () => {
    if (!formTrigger) {
      return (
        <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
          Form trigger metadata is unavailable.
        </div>
      );
    }
    const isCreatingForm = creatingTriggerKey === FORM_TRIGGER_KEY;

    if (!workflowInputEntries.length) {
      return (
        <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground space-y-2">
          <p className="font-medium">Form trigger requires workflow inputs</p>
          <p>Add Input blocks to your workflow to define the form fields that users will fill out.</p>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-5">
        <div className="rounded-lg border p-4 space-y-3 bg-muted/30">
          ...
          <p className="text-xs text-muted-foreground">
            Your workflow inputs will automatically become form fields. Form submissions will map directly to these inputs.
          </p>
          <div className="space-y-2">
            {workflowInputEntries.map(([inputName, inputDef]) => (
              <div key={inputName} className="flex items-center justify-between gap-4 text-xs">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{inputName}</span>
                  <span className="text-muted-foreground">
                    {inputDef.description || 'No description'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="e.g., Contact form for customer inquiries"
            onChange={(e) =>
              setFormConfig((prev) => ({ ...prev, description: e.target.value }))
            }
          />
          <p className="text-xs text-muted-foreground">
            This description will help you identify the form trigger later.
          </p>
        </div>

        <div className="rounded-lg border p-4 space-y-2 bg-amber-500/5 border-amber-500/20">
          <p className="text-xs font-medium">Public form URL</p>
          <p className="text-xs text-muted-foreground">
            After creating this trigger, you'll receive a public URL that anyone can use to submit the form and trigger this workflow.
          </p>
        </div>
        ...
        <Button
          ...
          disabled={!canCreateForm || isCreatingForm}
        >
          {isCreatingForm ? (
            <>
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
  ...
  const renderCreateForm = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {selectedTriggerKey === FORM_TRIGGER_KEY && renderFormForm()}
      </div>
    );
  };
